...
cp /path/to/JRVS/mcp_gateway/claude_config.json ~/.config/claude/mcp_servers.json
```
...
```json
{
  "mcpServers": {
    "jrvs": {
      "command": "python",
      "args": [
        "/path/to/JRVS/mcp_gateway/server.py"
      ],
      "env": {
        "PYTHONPATH": "/path/to/JRVS"
      }
    }
  }
}
...
Edit `/path/to/JRVS/config.py` to customize:
