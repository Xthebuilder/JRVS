...
class VisionModule(BaseExtension):
    ...
    async def initialize(self) -> bool:
        """Load all sub-components.  Returns True if ready, False if unavailable."""
        logger.info("Initializing VisionModule (backend=%s, source=%s)",
                    self._cfg.inference_backend, self._cfg.camera_source)

        # Start camera reader thread and check if it can actually get a frame
        self._camera.open()
        if self._camera.read_frame() is None:
            logger.debug("VisionModule: no camera frame available, disabling vision.")
            self._camera.close()
            return False
        ...
        try:
            ...
            logger.debug("VisionModule: backend unavailable (%s), disabling vision.", e)
            self._camera.close()
            return False
        ...
        try:
            ...
            logger.debug("VisionModule: memory init failed (%s), disabling vision.", e)
            self._camera.close()
            return False
