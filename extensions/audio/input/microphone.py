...
class MicrophoneCapture:
    ...
    def open(self) -> None:
        ...
        # Auto-detect the device's native sample rate if the requested rate fails.
        # This makes JRVS portable across machines with different audio hardware.
        sample_rate = self._resolve_sample_rate(device_id, self.sample_rate)
        if sample_rate != self.sample_rate:
            logger.warning(
                "Device does not support %d Hz; using native rate %d Hz instead "
                "(audio_module will resample to 16000 Hz for Whisper).",
                self.sample_rate, sample_rate,
            )
            self.sample_rate = sample_rate
    ...
    def flush(self) -> None:
        """Discard all buffered audio chunks (e.g., to clear speaker bleed)."""
        with self._lock:
            self._buffer.clear()
    ...
    def _resolve_sample_rate(
        device_id: Optional[Union[int, str]], requested: int
    ) -> int:
        """Return *requested* if the device supports it, else its native default rate."""
        if not _SD_AVAILABLE:
            return requested
        try:
            sd.check_input_settings(device=device_id, samplerate=requested, channels=1)
            return requested
        except Exception:
            try:
