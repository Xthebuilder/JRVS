...
Supported intents and example phrasings:
  ...
  UNMUTE            "unmute", "you can talk", "resume speaking"
  HELP              "what commands can I use", "help"
  EXIT_MODE         "exit command mode", "cancel", "never mind"
  FEEDBACK_BAD      "that was wrong", "bad answer", "you got that wrong"
"""
...
_RAW_PATTERNS: List[Tuple[str, str]] = [
    ...
    ("UNMUTE",
     r"\b(unmute|resume\s+speaking|start\s+talking\s+again|you\s+can\s+(talk|speak)"
     r"|speak\s+again|sound\s+on)\b"),

    # ── Help
    ("HELP",
     r"\b(help|what\s+commands?|what\s+can\s+you\s+do|command\s+list"
     r"|list\s+commands?|show\s+commands?|what\s+do\s+you\s+support)\b"),

    # ── Feedback — flag last response as bad for training loop
    ("FEEDBACK_BAD",
     r"\b(that\s+was\s+(wrong|bad|incorrect|off|not\s+right)"
     r"|wrong\s+answer|incorrect\s+answer|you\s+got\s+that\s+wrong"
     r"|bad\s+answer|that.{0,5}(wrong|incorrect|bad)"
     r"|not\s+what\s+i\s+(asked|wanted|meant)"
     r"|that.{0,10}not\s+(right|correct|accurate))\b"),
]
...
_COMMAND_MODE_TRIGGERS = re.compile(
    ...
    r"|jarvis\s+command\s+mode|switch\s+to\s+command\s+mode|commands?)\b",
    re.IGNORECASE,
)
