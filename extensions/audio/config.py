...
class AudioConfig:
    ...
    def __init__(self) -> None:
        ...
        dia = tts.get("dia", {})
        self.dia_model_id: str = _env(
            "tts_dia_model_id", dia.get("model_id", "nari-labs/Dia-1.6B")
        )
        self.dia_device: str = _env("tts_dia_device", dia.get("device", "cuda"))
        self.dia_compute_dtype: str = _env(
            "tts_dia_compute_dtype", dia.get("compute_dtype", "float16")
        )
        ...
        self.ambient_enabled: bool = _env("ambient_enabled", amb.get("enabled", True))
        self.ambient_min_words: int = _env(
            "ambient_min_words", amb.get("min_transcript_words", 3)
        )
        self.ambient_log_level: str = _env(
            "ambient_log_level", amb.get("log_level", "all")
        )
    ...
    def _validate(self) -> None:
        ...
        if self.tts_backend not in ("piper", "kokoro", "dia"):
            raise ValueError("tts.backend must be 'piper', 'kokoro', or 'dia'")
