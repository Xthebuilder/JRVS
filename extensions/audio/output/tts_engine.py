...
class TTSBackend(ABC):
    ...
    def stop_speaking(self) -> None:
        """Interrupt any ongoing audio playback. Override in backends that support it."""
        pass
...
def create_tts_backend(config) -> TTSBackend:
    ...
    if backend_name == "dia":
        from extensions.audio.output.dia_backend import DiaBackend
        logger.info(
            "TTS backend: Dia (%s on %s)", config.dia_model_id, config.dia_device
        )
        return DiaBackend(
            model_id=config.dia_model_id,
            device=config.dia_device,
            compute_dtype=config.dia_compute_dtype,
            speed=config.tts_speed,
        )

    raise ValueError(
        f"Unknown TTS backend: '{backend_name}'. Choose 'piper', 'kokoro', or 'dia'."
    )
