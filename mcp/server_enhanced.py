...
from lib.music_tools import download_song as _download_song, list_downloads as _list_downloads
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Release resource slot only if it was acquired
                if slot_acquired:
...
@mcp.tool()
async def download_music(url: str) -> dict:
    """
    Download a YouTube video (or playlist) as MP3 using the local ytdl script.

    Skips the download if the file already exists in ~/Music/ytdl/.
    The URL is passed directly to the subprocess — shell injection is not
    possible because shell=False is enforced in the underlying helper.

    Args:
        url: Full YouTube (or compatible) URL to download.

    Returns a dict with:
        status    — "already_exists" | "success" | "error"
        message   — human-readable result
        file      — absolute path to the downloaded file (if known)
        returncode — ytdl exit code, or None for pre-flight results
    """
    return _download_song(url)


@mcp.tool()
async def list_music_downloads() -> list:
    """
    List all MP3 files currently saved in ~/Music/ytdl/.
