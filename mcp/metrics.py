...
class MetricsCollector:
    ...
    def get_tool_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        with self._lock:
            # Call the stats logic inline to avoid re-acquiring the non-reentrant lock
            result = {}
            for tool in list(self.request_count.keys()):
                total = self.request_count.get(tool, 0)
                errors = self.error_count.get(tool, 0)
                durations = self.duration_buckets.get(tool, [])
                success_rate = ((total - errors) / total * 100) if total > 0 else 0
                stats = {
                    "total_requests": total,
                    "successful_requests": total - errors,
