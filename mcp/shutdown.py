...
class ShutdownHandler:
    ...
    async def shutdown(self):
        ...
        # Signal the event loop to stop instead of sys.exit() —
        # sys.exit() inside an asyncio task raises SystemExit which
        # the event loop catches as an unhandled task exception and
        # does NOT propagate, so the process wouldn't actually exit.
        try:
