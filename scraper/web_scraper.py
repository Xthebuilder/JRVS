...
class WebScraper:
        ...
        try:
            ...
            async with session.get(url) as response:
                if response.status != 200:
                    log.warning("HTTP %s scraping %s", response.status, url)
                    return None
            ...
            # Connection-level error — session may be in a broken state; reset it
            log.warning("Connection error scraping %s: %s — resetting session", url, e)
    ...
    def _clean_text(self, text: str) -> str:
        ...
        # Collapse horizontal whitespace only (preserve paragraph breaks)
        text = re.sub(r'[^\S\n]+', ' ', text)
    ...
    async def scrape_and_store(self, url: str) -> Optional[int]:
        ...
        # Deduplication guard — don't re-ingest a URL already in the knowledge base
        if await _db.check_document_exists(url):
            log.debug("Already ingested, skipping: %s", url)
            return None
        ...
        if len(scraped_data['content']) < 100:
            log.warning("Content too short for %s, skipping", url)
            return None
        ...
        try:
            ...
            log.info("Stored %s as document %d", url, document_id)
            return document_id

        except Exception as e:
            log.error("Error storing %s: %s", url, e)
            return None
    ...
    async def search_and_scrape(self, query: str, search_engine: str = "auto",
        ...
        return await self.scrape_multiple_urls(urls, max_concurrent=3)
        ...
        while to_visit and len(visited) < max_pages:
            ...
            if depth < max_depth:
                try:
                    ...
                    log.warning("Error extracting links from %s: %s", url, e)
            
            # Small delay to be respectful
            await asyncio.sleep(1)
        
        log.info("Scraped %d pages from %s", len(document_ids), start_url)
        return document_ids
