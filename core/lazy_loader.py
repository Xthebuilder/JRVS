...
class LazyLoader(Generic[T]):
    ...
    def __init__(self, loader_func: Callable[[], T], ttl: Optional[float] = None):
        ...
        self._loading = False
        # Defer asyncio primitives until first use so this class can be instantiated
        # at module level (outside an event loop) on Python 3.10+.
        self._lock: Optional[asyncio.Lock] = None
        self._load_complete: Optional[asyncio.Event] = None

    def _ensure_primitives(self) -> None:
        """Lazily create event-loop-bound primitives on first async call."""
        if self._lock is None:
            self._lock = asyncio.Lock()
        if self._load_complete is None:
...
class ResourcePool:
    ...
    def __init__(self, max_size: int = 10):
        ...
        self._pool = {}
        self._usage_count = {}
        self._lock: Optional[asyncio.Lock] = None  # deferred — safe for module-level init
    ...
    async def _evict_lru(self):
        ...
        if hasattr(resource, 'cleanup'):
            try:
                ...
                log.warning("Error cleaning up resource %s: %s", lru_key, e)
            ...
            for key, resource in self._pool.items():
                if hasattr(resource, 'cleanup'):
                    try:
                        ...
                        log.warning("Error cleaning up resource %s: %s", key, e)
                ...
                try:
                    ...
                    if attempt < max_retries:
                        log.warning("Attempt %d/%d failed: %s — retrying in %.1fs",
                                    attempt + 1, max_retries + 1, e, current_delay)
                        await asyncio.sleep(current_delay)
                        current_delay *= backoff
                    else:
                        log.error("All %d attempts failed for %s", max_retries + 1,
