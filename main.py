...
async def main():
    ...
    # ── Cosine Bouncer startup check ─────────────────────────────────────────
    from core.security import vibe_checker, SECURITY_MODEL, VIBE_THRESHOLD
    bouncer_ok = vibe_checker.warm_up()
    if bouncer_ok:
        theme.print_success(
            f"Cosine Bouncer armed "
            f"(model={SECURITY_MODEL}, device={vibe_checker._device}, threshold={VIBE_THRESHOLD})"
        )
    else:
        theme.print_warning(
            f"[SECURITY] Cosine Bouncer failed to load: {vibe_checker._load_error}"
        )
        theme.print_warning(
            "[SECURITY] Prompt-injection protection is DISABLED. "
            "Fix the error above before handling untrusted input."
        )
    # ─────────────────────────────────────────────────────────────────────────
