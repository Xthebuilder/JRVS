...
class MCPClient:
    ...
    async def _load_config(self):
        ...
        # Inject $JRVS_DIR so client_config.json can reference the project root portably
        os.environ.setdefault("JRVS_DIR", str(self.config_path.parent.parent.resolve()))

        with open(self.config_path, 'r') as f:
            config_data = json.load(f)

        for name, server_data in config_data.get("mcpServers", {}).items():
            # Expand ~ and $ENV_VARS (including $JRVS_DIR) in command and all args
            def expand(s: str) -> str:
                return os.path.expandvars(os.path.expanduser(s))

            # Merge config env with current process env so the child process
            # keeps PATH, HOME, etc.  Then expand $VAR refs in the override values.
            raw_env = server_data.get("env") or {}
            merged_env = {**os.environ, **{k: expand(v) for k, v in raw_env.items()}}

            self.servers[name] = MCPServerConfig(
                name=name,
                command=expand(server_data["command"]),
                args=[expand(a) for a in server_data.get("args", [])],
                env=merged_env,
            )
    ...
    async def read_resource(self, server_name: str, uri: str) -> Any:
        """Read an MCP resource by URI (e.g. screenshot://name from puppeteer)."""
        if server_name not in self.connections:
            raise ValueError(f"Server '{server_name}' not connected")
        connection = self.connections[server_name]
    ...
    async def disconnect_server(self, server_name: str, timeout: float = 3.0):
        """
        Disconnect from a specific MCP server with a timeout.
    ...
    async def cleanup(self):
        """Disconnect from all MCP servers with per-server timeouts."""
        for server_name in list(self.connections.keys()):
            try:
                # Puppeteer gets a shorter timeout — Chromium can linger
                timeout = 2.0 if server_name == "puppeteer" else 5.0
                await self.disconnect_server(server_name, timeout=timeout)
