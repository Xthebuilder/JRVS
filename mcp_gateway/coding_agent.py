...
class JARCORE:
    ...
    def set_llm_client(self, client) -> None:
        """Wire the CLI's configured LLM client so JARCORE doesn't hardcode Ollama."""
        self._llm_client = client
        log.info("JARCORE: LLM client injected (%s)", type(client).__name__)
    ...
    def _client(self):
        ...
        if self._llm_client is None:
            raise RuntimeError(
                "LLM client not injected — call set_llm_client() before using JARCORE."
            )
    ...
    async def write_file(
        ...
        try:
            ...
            return {
                ...
                "lines": len(content.split('\n')),
                "backed_up": backup and file_existed
            }
...
Cursor at position: {cursor_position if cursor_position else "end"}"""
    ...
    def get_edit_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        recent = list(self.edit_history)[-limit:] if len(self.edit_history) > limit else list(self.edit_history)
        return [
            {
