...
# These imports require faiss/sentence-transformers which are no longer in
# requirements now that Mem0 is the active backend.  Guard them so the module
# still loads cleanly on a fresh install that only has mem0ai.
try:
    ...
    embedding_manager = None  # type: ignore[assignment]
    reranker = None           # type: ignore[assignment]


# Kept for backward compatibility — used when RAG_BACKEND=faiss.
class RAGRetriever:
    def __init__(self):
    ...
    async def add_document(self, content: str, title: str = "", url: str = "",
        ...
        logger.info("Added document '%s' with %d chunks", title, len(chunks))
        return document_id
    ...
    async def embed_conversation(self, user_message: str, ai_response: str,
        ...
        try:
            ...
            if vector_store is None:
                logger.warning("Vector store not available — skipping FAISS embed for conversation %d", conversation_id)
                return
            await vector_store.add_documents([conversation_text], [metadata])
            await db.mark_conversation_embedded(conversation_id)
        except Exception as e:
            logger.error("Failed to embed conversation %d: %s", conversation_id, e)

    async def embed_pending_conversations(self, session_id: str = None) -> int:
        """Embed all unembedded conversations. If session_id is given, only that session."""
        await self.initialize()
        unembedded = await db.get_unembedded_conversations(limit=200)
        if session_id:
            unembedded = [c for c in unembedded if c.get('session_id') == session_id]
        count = 0
        for conv in unembedded:
            ...
            count += 1
        if count:
            logger.info("Catch-up: embedded %d pending conversation(s) into FAISS", count)
        return count
    ...
    async def retrieve_context(self, query: str, session_id: str = None) -> str:
        ...
        try:
            ...
            # ── 3. Session-aware conversation scoring ────────────────────────
            # Current-session turns are already in conversation_history, so
            # deprioritize them here to surface cross-session memory first.
            if session_id and conv_candidates:
                conv_candidates = [
                    (text, score * 0.4 if meta.get('session_id') == session_id else score, meta)
                    for text, score, meta in conv_candidates
                ]
                conv_candidates.sort(key=lambda x: x[1], reverse=True)

            # ── 4. Cross-encoder re-rank documents ───────────────────────────
            if doc_candidates:
            ...
            # ── 5. MMR — balance relevance vs diversity ───────────────────────
            if doc_candidates:
            ...
            # ── 6. Format — label cross-session vs earlier-this-session ──────
            context_parts = []
            if conv_candidates:
                cross_session = [(t, s, m) for t, s, m in conv_candidates
                                 if m.get('session_id') != session_id]
                this_session  = [(t, s, m) for t, s, m in conv_candidates
                                 if m.get('session_id') == session_id]
                if cross_session:
                    formatted = self._format_conversation_context_semantic(cross_session)
                    if formatted:
                        context_parts.append(f"Memory from past sessions:\n{formatted}")
                if this_session:
                    formatted = self._format_conversation_context_semantic(this_session)
                    if formatted:
            ...
            if elapsed > TIMEOUTS["context_building"]:
                logger.warning("Context building took %.2fs", elapsed)
...
# ---------------------------------------------------------------------------
# Factory — select backend from config.RAG_BACKEND
# ---------------------------------------------------------------------------
from config import RAG_BACKEND  # noqa: E402


def _create_retriever():
    """Return the RAG backend selected by config.RAG_BACKEND."""
    if RAG_BACKEND == "faiss":
        if vector_store is None or embedding_manager is None:
            raise ImportError(
                "RAG_BACKEND='faiss' requires faiss-cpu and sentence-transformers. "
                "Install them or set RAG_BACKEND=mem0."
            )
        logger.info("RAG backend: FAISS (hybrid search + cross-encoder reranking)")
        return RAGRetriever()
    else:
        from rag.mem0_retriever import Mem0RAGRetriever  # noqa: E402
        logger.info("RAG backend: Mem0 (Ollama + Qdrant-local)")
        return Mem0RAGRetriever()


rag_retriever = _create_retriever()
